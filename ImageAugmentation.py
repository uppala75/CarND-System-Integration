...
# Load the images
    ...
    local_path = './data/sim-slack/Red/' + filename
...
with open('./data/sim-slack/Yellow/yellowimgs.csv') as csvfile:
    reader = csv.reader(csvfile)
    for line in reader:
        lines_yellow.append(line)

# Load each row into line
for line in lines_yellow:
# Load the images
    filename = line[0]
    local_path = './data/sim-slack/Yellow/' + filename
    image = cv2.imread(local_path)
    images.append(image)
    label = line[1]
    labels.append(label)
...
with open('./data/sim-slack/Green/greenimgs.csv') as csvfile:
    reader = csv.reader(csvfile)
    for line in reader:
        lines_green.append(line)
		
# Load each row into line
for line in lines_green:
# Load the images
    filename = line[0]
    local_path = './data/sim-slack/Green/' + filename
    image = cv2.imread(local_path)
    images.append(image)
    label = line[1]
    labels.append(label)
		
with open('./data/sim-slack/None/noneimgs.csv') as csvfile:
    reader = csv.reader(csvfile)
    for line in reader:
        lines_none.append(line)

# Load each row into line
for line in lines_none:
# Load the images
    filename = line[0]
    local_path = './data/sim-slack/None/' + filename
    image = cv2.imread(local_path)
    images.append(image)
    label = line[1]
    labels.append(label)
